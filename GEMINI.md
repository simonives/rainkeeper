# GEMINI.md — Rainkeeper

Rainkeeper is Simon's own open-source MCP server for Raindrop.io, described here for Gemini operating directly on this repo via Antigravity. It fixes two bugs in existing community alternatives: it works on free Raindrop accounts (the official MCP requires Pro), and collection moves actually work (correct `{"collection": {"$id": n}}` request body format). MIT licence.
...
## Precedence
This file is authoritative for all rainkeeper work — no parent GEMINI.md exists inside this repo. General developer and tool conventions apply where not overridden here; this file overrides defaults on rainkeeper-specific technical and process matters (e.g. mandatory TDD) regardless of recency.
