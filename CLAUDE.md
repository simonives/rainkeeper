# CLAUDE.md — Rainkeeper

Rainkeeper is Simon's own open-source MCP server for Raindrop.io. It fixes two bugs in existing community alternatives: it works on free Raindrop accounts (the official MCP requires Pro), and collection moves actually work (correct `{"collection": {"$id": n}}` request body format). MIT licence.
...
## Precedence
This file is authoritative for all rainkeeper work — no parent CLAUDE.md exists inside this repo. Global `~/.claude/CLAUDE.md` DOCTRINE and preferences apply where not overridden here; this file overrides the global defaults on rainkeeper-specific technical and process matters (e.g. mandatory TDD) regardless of recency.
...
- The Superpowers plugin (`superpowers@claude-plugins-official`) methodology is mandatory for all work on this project.
